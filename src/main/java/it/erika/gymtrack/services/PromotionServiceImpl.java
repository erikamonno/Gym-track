...
import it.erika.gymtrack.exceptions.PromotionNotFound;
...
public class PromotionServiceImpl implements PromotionService {
    ...
    public PromotionServiceImpl(PromotionRepository repository, ReferenceMapper referenceMapper, PromotionMapper mapper) {
    ...
    private void checkRangeDate(PromotionDto dto, UUID subscriptionTypeId, UUID id) {
        var existsOverlappingPromotions = repository.exists(new OverlappingPromotionSpecification(id, subscriptionTypeId, dto.getValidTo(), dto.getValidFrom()));
    ...
    public List<PromotionDto> getAllPromotions(UUID subscriptionTypeId) {
        ...
        return repository.findBySubscriptionTypeId(subscriptionTypeId).stream().map(promotion -> mapper.toDto(promotion)).toList();
    }
    ...
    public PromotionDto getPromotion(UUID id) {
        ...
        if(oEntity.isEmpty()) {
            throw new PromotionNotFound(HttpStatus.NOT_FOUND, "Promotion not found");
        }
